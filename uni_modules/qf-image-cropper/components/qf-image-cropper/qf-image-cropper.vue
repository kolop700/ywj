	...
	export default {
		...
		methods: {
			...
			canvasToTempFilePath(canvas, canvasId) {
				...
				uni.canvasToTempFilePath({
					...
					success: (res) => {
						...
						const compressedPath = url.createObjectURL(
							new Blob([u8arr], {
								type: mime
							})
						);
						this.handleImage(compressedPath);
						// #endif
			...
			handleImage(tempFilePath) {
				// #ifdef H5
				if (tempFilePath.indexOf('data:image') !== -1) {
					// 处理base64数据
					...
					uni.hideLoading();
					this.$emit('crop', { tempFilePath: compressedPath });
				} else {
					// 直接使用本地路径
					uni.hideLoading();
				...
				// #ifdef APP-PLUS
				// APP平台下使用plus.zip.compressImage
				plus.zip.compressImage({
					...
					uni.hideLoading();
					this.$emit('crop', { tempFilePath: event.target });
				}, (error) => {
				...
				// #ifdef MP
				// 小程序平台使用uni.compressImage
				uni.compressImage({
					src: tempFilePath,
					quality: parseInt(this.quality * 100),
					success: (res) => {
						uni.hideLoading();
						this.$emit('crop', { tempFilePath: res.tempFilePath });
					},
					fail: (err) => {
